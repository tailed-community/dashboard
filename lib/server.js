...
export async function paraglideMiddleware(request, resolve, callbacks) {
    ...
    if (request.headers.get("Sec-Fetch-Dest") === "document" &&
        ...
        if (normalizeURL(localizedUrl.href) !== normalizeURL(request.url)) {
            ...
            const response = new Response(null, {
                status: 307,
                headers: Object.assign({ Location: localizedUrl.href }, headers),
            });
            callbacks === null || callbacks === void 0 ? void 0 : callbacks.onRedirect(response);
            return response;
        }
    }
    ...
    const response = await ((_a = runtime.serverAsyncLocalStorage) === null || _a === void 0 ? void 0 : _a.run({ locale, origin, messageCalls }, () => resolve({ locale, request: newRequest })));
    // Only modify HTML responses
    if (runtime.experimentalMiddlewareLocaleSplitting &&
        ((_b = response.headers.get("Content-Type")) === null || _b === void 0 ? void 0 : _b.includes("html"))) {
        ...
        for (const messageCall of Array.from(messageCalls)) {
            ...
            messages.push(`${id}: ${(_c = compiledBundles[id]) === null || _c === void 0 ? void 0 : _c[locale]}`);
        }
