    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          labelKey: "Languages",
          descKey: "Visualization of candidates' language proficiencies",
          href: "/catalog",
        },
      ],
    },
        ...
        <div>
          <div className="flex flex-col space-2">
            {flatFeatures.map((f, idx) => {
              const chipStyle = { color: hexToRgba(f.color, 0.75) };
              return (
                <a
                  key={idx}
                  href={f.href}
    ...
    <div className="w-full grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <div className="space-y-2">
            ...
            >
              ...
              <p className="text-sm leading-5 text-muted-foreground min-h-[2.5rem] overflow-hidden [display:-webkit-box] [-webkit-line-clamp:2] [-webkit-box-orient:vertical]">
                Where Franck's vision met purpose — Tail'ed was born to redefine
                hiring.
              </p>
            </a>
            <a
          ...
          <div className="space-y-2">
            <a
              ...
              className="block rounded-xl px-4 py-3 hover:bg-muted/60 focus:outline-none focus:ring-2 focus:ring-ring"
            >
              <div className="text-sm font-medium text-foreground">Careers</div>
              <p className="text-sm leading-5 text-muted-foreground min-h-[2.5rem] overflow-hidden [display:-webkit-box] [-webkit-line-clamp:2] [-webkit-box-orient:vertical]">
      ...
      <div className="rounded-2xl p-6">
        ...
        >
          ...
          />
          <p className="text-sm text-muted-foreground">
            Providing the latest information for the HR industry, tech industry,
            and tech students
          </p>
        </a>
        <a
      ...
      <div className="rounded-2xl p-6">
        ...
        <a
          ...
          className="block rounded-xl px-4 py-3 hover:bg-muted/60 focus:outline-none focus:ring-2 focus:ring-ring"
        >
          <div className="text-sm font-medium text-foreground">Comics</div>
          <p className="text-sm leading-5 text-muted-foreground min-h-[2.5rem] overflow-hidden [display:-webkit-box] [-webkit-line-clamp:2] [-webkit-box-orient:vertical]">
            Read Fox Comics
          </p>
        </a>
        ...
        >
          <div className="text-sm font-medium text-foreground">Fox Shop</div>
          <img
            src="/fox-shop.png"
            alt="Fox Shop"
          ...
          <div className="flex w-full z-50">
            <div className="flex flex-1 min-w-0 items-center gap-8">
              ...
              <ul className="hidden md:flex items-center gap-1">
                {navItems.map((item, idx) =>
                  item.type === "link" ? (
                    <li key={idx}>
                      <NavLink href={item.href}>{item.labelKey}</NavLink>
                    </li>
                  ) : (
                    <li
                      key={idx}
                    ...
                    >
                      <div
                        ...
                        aria-haspopup="true"
                        onFocus={() => setOpenMenu(item.labelKey)}
                        className={cn(
            ...
            <div className="flex items-center gap-3">
              <a
                ...
                className="hidden md:inline-flex items-center gap-2 text-sm rounded-lg px-4 py-3 hover:bg-muted/60"
                aria-label="GitHub repository"
              >
                <FaGithub className="h-4 w-4" /> GitHub
              </a>
              <div className="h-6 w-2 text-black hidden md:block">
                <Separator orientation="vertical" className="bg-gray-700" />
              </div>
              <a
                href="/sign-in"
                className="hidden md:inline-block text-sm rounded-lg px-4 py-3 hover:bg-[#f8d1b1]"
              >
                Log in
              </a>
              ...
              <Sheet>
                ...
                <SheetContent side="left" className="p-0 flex flex-col">
                  <SheetHeader className="px-4 py-3 shrink-0">
                    <SheetTitle className="sr-only">main_navigation</SheetTitle>
                    ...
                    <Accordion type="single" collapsible className="w-full">
                      {navItems.map((item, idx) =>
                        item.type === "link" ? (
                          ...
                          <AccordionItem key={idx} value={item.labelKey}>
                            <AccordionTrigger className="py-3 text-base">
                              {item.labelKey}
                            </AccordionTrigger>
                            <AccordionContent className="pb-4">
                              {item.labelKey === "Product" ? (
            ...
            >
              {openMenu &&
                (() => {
                  const active =
                    navItems.find(
                      (it): it is Extract<NavItem, { type: "menu" }> =>
                        it.type === "menu" && it.labelKey === openMenu
                    ) ?? null;
