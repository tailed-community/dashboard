...
export function AttendeeDetailsDrawer({
  ...
  const handleStatusUpdate = async (newSt: string) => {
    ...
    try {
      ...
      onRegistrationUpdated({
        ...
        approvedAt: newSt === "confirmed" ? new Date().toISOString() : registration.approvedAt,
      });
  ...
  return (
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent side="right" className="w-[600px] overflow-y-auto p-6">
          ...
          {/* Current Status */}
              <div className="rounded-lg border p-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {registration.approvedAt && registration.status === "confirmed" && (
                    <p className="text-sm text-muted-foreground">
                      Approved on {formatDate(registration.approvedAt, "MMM dd, yyyy 'at' hh:mm a")}
                    </p>
                  )}
                </div>
              </div>
          ...
          {/* Registration Info */}
          <div className="rounded-lg border p-4">
            <h3 className="mb-3 font-semibold text-sm">Registration Info</h3>
