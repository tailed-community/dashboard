...
export default function ApplicationForm({
  ...
  function prefillFormData(
    ...
    return {
      ...
      githubUsername: formData.githubUsername || profile.githubUsername || "",
      devpostUsername:
    ...
    async function fetchProfileAndPrefill() {
      try {
        ...
        if (!ignore && profile && Object.keys(profile).length > 0) {
          setFormData((prev) => ({
            ...prev,
            ...prefillFormData(prev as ApplicationFormData, profile),
          }));
        }
      } catch (err) {
        ...
        try {
          ...
          if (
            ...
            if (credential) {
              ...
              const userCred = GithubAuthProvider.credentialFromResult(result);
    ...
    <div className="container mx-auto py-8 px-4">
      ...
      <Card className="mx-auto max-w-3xl">
        <CardHeader>
          ...
          <CardDescription>
            Thank you for your interest in this position. Please complete this
            form to apply.
          </CardDescription>
          <Steps currentStep={currentStep} className="mt-4">
            <Step icon={<User size={18} />} title="Personal Info" />
            <Step icon={<Github size={18} />} title="GitHub Profile" />
            <Step icon={<Award size={18} />} title="Hackathons" />
            <Step icon={<Briefcase size={18} />} title="Job Details" />
          </Steps>
        </CardHeader>
        <CardContent>
