  ...
  return (
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        {devpostError && (
          <p className="text-sm text-destructive mt-1">{devpostError}</p>
        )}
      </div>
      ...
      <div className="p-4 bg-muted rounded-lg">
        <p className="text-sm">
          <strong>Note:</strong> If you don't have a Devpost profile or haven't
          participated in hackathons, you can leave this field empty. This won't
    ...
    <div className="rounded-lg border p-4">
            ...
            <div className="space-y-3">
              {devpostProfile.skills?.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {devpostProfile.skills.map((skill, index) => (
              ...
              {devpostProfile.interests?.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {devpostProfile.interests.map((interest, index) => (
      ...
      <div className="space-y-4">
        {!devpostProfile.projectsAndHackathons ||
          ...
          <p className="text-sm text-muted-foreground">No hackathons found</p>
        ) : (
          devpostProfile.projectsAndHackathons
            .slice(0, 5)
            .map((entry, index) => (
              <div key={index} className="border-t pt-3">
                <div className="flex justify-between items-start">
                  ...
                  {entry.win && entry.win !== "no" && (
                    <Badge variant="success">
                      {entry.win === "yes" ? "Winner" : entry.win}
                    </Badge>
                  )}
                </div>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  ...
                  {/* Prizes won */}
                  {entry.prizes && entry.prizes.length > 0 && (
                    <div className="mt-2 w-full">
                      <p className="text-xs font-medium text-muted-foreground">
                        Prizes:
                      </p>
                      <div className="flex flex-wrap gap-1 mt-1">
                        {entry.prizes.map((prize, prizeIndex) => (
                          <Badge
                            key={prizeIndex}
                            variant="outline"
                            className="text-xs"
                          >
                            {prize.prize_text.replace(/^Winner/, "")}
                          </Badge>
                        ))}
                      </div>
                    </div>
                    ...
                    entry.hackathon_details.description && (
                      <details className="text-xs w-full mt-1">
                        ...
                        <div className="mt-2 pl-2 border-l-2 border-muted">
                          <p>{entry.hackathon_details.description}</p>
                          {entry.hackathon_details.deadline &&
                            entry.hackathon_details.deadline !==
                            "No Deadline Info" && (
                              <p className="mt-1">
                                Deadline: {entry.hackathon_details.deadline}
                              </p>
                            )}
                        </div>
                      </details>
        ...
        {devpostProfile.projectsAndHackathons &&
          devpostProfile.projectsAndHackathons.length > 5 && (
            <p className="text-sm text-muted-foreground text-center">
              +{devpostProfile.projectsAndHackathons.length - 5} more projects
            </p>
          )}
      </div>
      ...
      {devpostProfile.achievements && (
        <>
          ...
          <div className="grid grid-cols-2 gap-4 text-center">
            ...
            <div className="p-2 bg-muted rounded-md">
              <p className="text-lg font-semibold">
                {devpostProfile.achievements.firstPlaceWins || 0}
              </p>
              <p className="text-xs text-muted-foreground">First Place Wins</p>
            </div>
          </div>
        </>
      )}
      ...
      {devpostProfile.hackathons?.participationByYear &&
        Object.keys(devpostProfile.hackathons.participationByYear).length >
        0 && (
          <>
            <Separator className="my-4" />
            <h4 className="font-medium mb-3">Yearly Participation</h4>
            <div className="flex flex-wrap gap-3">
              {Object.entries(devpostProfile.hackathons.participationByYear)
                ...
                .map(([year, count]) => (
                  ...
                  >
                    ...
                    <p className="text-sm">
                      {count} {count === 1 ? "hackathon" : "hackathons"}
                    </p>
                  </div>
                ))}
            </div>
