  ...
  return (
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        {githubError && (
          <p className="text-sm text-destructive mt-1">{githubError}</p>
        )}
        {isAnonymous && !githubProfile && !isLoadingGithub && (
          <p className="text-sm text-muted-foreground mt-1">
            Connect your GitHub account to enhance your application with your
            coding activity and projects.
          </p>
        )}
      </div>
    ...
    <div className="rounded-lg border p-4 space-y-4">
      <div className="flex items-start gap-4">
        <Avatar className="h-16 w-16">
          <AvatarImage src={githubProfile.avatarUrl} alt={githubProfile.name} />
          <AvatarFallback>
            {githubProfile.username.substring(0, 2).toUpperCase()}
          </AvatarFallback>
        </Avatar>
        <div className="flex-1">
          <h3 className="text-lg font-semibold">{githubProfile.name}</h3>
          ...
          <div className="mt-2 flex flex-wrap gap-3 text-sm text-muted-foreground">
            ...
            <span className="flex items-center">
              ...
              Joined {new Date(githubProfile.createdAt).toLocaleDateString()}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4 text-center">
        <div>
          <p className="text-lg font-semibold">{githubProfile.repoCount}</p>
          <p className="text-xs text-muted-foreground">Repositories</p>
        </div>
        <div>
          <p className="text-lg font-semibold">{githubProfile.followers}</p>
        ...
        <div>
          <p className="text-lg font-semibold">{githubProfile.maxStreak}</p>
          ...
          {githubProfile.languageDistribution && (
            <div className="mt-3">
              ...
              <div className="space-y-2 mt-2">
                {githubProfile.languageDistribution.map((lang, index) => (
                  <div key={index} className="space-y-1">
                    ...
                    <div className="flex justify-between text-xs text-muted-foreground">
                      <span>
                        {lang.projects}{" "}
                        {lang.projects === 1 ? "project" : "projects"}
                      </span>
                    </div>
                  </div>
                ))}
        ...
        <>
          <div>
            <h4 className="text-sm font-medium mb-3 flex items-center">
              <Calendar className="h-4 w-4 mr-1" /> Contribution Activity
            </h4>
              ...
              <div className="min-w-[640px]">
                ...
                <div className="flex justify-between mt-2 text-xs text-muted-foreground">
                  <div>
                    ...
                    <div>
                      {githubProfile.mostProductiveDay?.date}:{" "}
                      {githubProfile.mostProductiveDay?.contributions}{" "}
                      contributions
                    </div>
                  </div>
                    ...
                    <div>
                      {(
                        githubProfile.weeklyContributions?.reduce(
                          ...
                          0
                        ) / (githubProfile.weeklyContributions?.length || 1)
                      ).toFixed(1)}{" "}
                      contributions
                    </div>
                  </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Contributions / Day</p>
          </div>
          <div>
            <p className="text-sm">{githubProfile.activeDaysPerWeek}</p>
            <p className="text-xs text-muted-foreground">Active Days / Week</p>
          </div>
          <div>
          ...
          <div>
            <p className="text-sm">{githubProfile.pullRequestsCount}</p>
      ...
      {githubProfile.mostStarredRepos.length > 0 && (
        <>
          <Separator />
          <div>
            <h4 className="text-sm font-medium mb-2">Top Repositories</h4>
            <div className="space-y-2">
              {githubProfile.mostStarredRepos.slice(0, 3).map((repo, index) => (
