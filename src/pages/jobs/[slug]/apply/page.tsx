...
export default function ApplyJobPage() {
    ...
    async function handleApplicationSubmit(formData: any) {
        try {
            ...
            // If application submitted successfully and Devpost profile was provided, save it to user's account
            if (formData.devpost && formData.devpost.username) {
                try {
                    const saveResponse = await apiFetch(`/devpost/save`, {
                        ...
                        body: JSON.stringify({
                            devpostProfile: formData.devpost,
                        }),
                    });

                    if (!saveResponse.ok) {
                        console.warn(
                            "Devpost profile could not be saved to account"
                        );
                    }
                } catch (saveError) {
                    // Don't block the user if save fails, silently log it
                    console.warn("Error saving Devpost profile:", saveError);
                }
            }
