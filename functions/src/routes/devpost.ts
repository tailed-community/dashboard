...
async function callDevpostAPI(endpoint: string) {
    ...
    const url =
        "https://3fqfekw0u5.execute-api.us-east-1.amazonaws.com/Prod" +
        endpoint;
    ...
    try {
        ...
        if (response.status !== 200) {
            ...
            return {
                success: false,
                errors: [
                    `Couldn't call the Devpost API (Status: ${response.status})`,
                ],
            };
        }
...
async function fetchHackathonDetails(hackathonId: string): Promise<any> {
    try {
        ...
        if (!response.success) {
            logger.error(
                `Failed to fetch hackathon details for ${hackathonId}`
            );
            return null;
        }
        ...
        logger.error(
            `Error fetching hackathon details for ${hackathonId}:`,
            error
        );
        return null;
    }
...
// POST /devpost/profile - Fetch comprehensive Devpost data for a given username (verification only)
router.post("/profile", async (req, res) => {
    try {
        ...
        if (!response.success) {
            return res.status(400).json({
                success: false,
                error: "Failed to fetch data from Devpost. Please check the username and try again.",
            });
        }
        ...
        const combinedProjectsAndHackathons = await Promise.all(
            projects.map(async (project: any) => {
                ...
                const associatedHackathon = hackathons.find(
                    (hackathon: any) =>
                        hackathon.name === project.hackathon_name
                );
                ...
                try {
                    ...
                    const projectId =
                        project.link || project.url?.split("/").pop();
                    if (projectId) {
                        enrichedProjectData =
                ...
                if (associatedHackathon) {
                    ...
                    if (hackathonName) {
                        hackathonDetails =
                            enrichedHackathons.find(
                                (h) => h.id === hackathonName
                            ) || null;
                    }
                }
                ...
                return {
                    ...
                    project_link:
                        project.url ||
                        `https://devpost.com/software/${project.link}` ||
                        "",
                    github_link:
                        project.github_url ||
                    ...
                    year: associatedHackathon
                        ? new Date(
                              associatedHackathon.date || ""
                          ).getFullYear() || null
                        : null,
                    technologies:
                        project.technologies ||
                        enrichedProjectData?.built_with ||
                        [],
                    // Add enriched project data
                    description:
                        enrichedProjectData?.description ||
                        project.tagline ||
                        "",
                    members: enrichedProjectData?.created_by || [],
                    built_with: enrichedProjectData?.built_with || [],
                    detailed_technologies:
                        enrichedProjectData?.technologies || [],
                    images:
                        enrichedProjectData?.gallery?.map(
                            (item: any) => item.url
                        ) || [],
                    video:
                    ...
                    hackathon_details: hackathonDetails
                        ? {
                              ...
                              sponsors:
                                  hackathonDetails.sponsors?.slice(0, 5) || [],
                          }
                        : null,
                };
            })
